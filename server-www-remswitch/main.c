 ...
 *
 * Ethernet remote device to control a relay connected via PD7.
 * Optionally you can connect a push button between PD6 and GND to
 * control the state of the relay not only via web browser or UDP but
 * as well locally.
 *
...
UDP:
                ...
                if(eth_type_is_ip_and_my_ip(buf,plen)==0){
                        ...
                        if (debounce==0 && bit_is_clear(PIND,PIND6)){
                                debounce=0x9fff; // debounce button press
                                // toggle on push button press:
                                if (PORTD & (1<<PORTD7)){
                                        PORTD &= ~(1<<PORTD7);// transistor off
                                        LEDOFF;
                                }else{
