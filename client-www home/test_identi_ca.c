 ...
 * web server at WEBSERVER_VHOST (identi.ca)
...
#define WEBSERVER_VHOST "identi.ca"
// Username and password for identi.ca:
...
#define BLOGGACCOUNT "Authorization: Basic ZXxoYmVhcj80dXhXYW41Nw=="
...
// WEBSERVER_VHOST.
static char password[]="secret"; // the password string for this web server (not identi.ca), (only a-z,0-9,_ characters):
...
// global string buffer for identi.ca message:
...
static volatile uint8_t start_web_client=0;  // 0=off but enabled, 1=send mail, 2=main sending initiated, 3=identi.ca was sent OK, 4=diable identi.ca notify
        ...
        while(1){
                ...
                if(plen==0){
                        ...
                        if (start_web_client==1){
                                ...
                                client_http_post(PSTR("/api/statuses/update.xml"),"",PSTR(WEBSERVER_VHOST),PSTR(BLOGGACCOUNT),statusstr,&browserresult_callback,otherside_www_ip,gwmac);
                        }
